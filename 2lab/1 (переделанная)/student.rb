...
class Student < Student_abstract
  ...
  def getInfo
    "fullname: #{self.last_name} #{self.first_name[0]}. #{self.patronymic[0]}., #{if git_valid? then "git: #{self.git}, " end}#{Student_abstract.any_contact(phone: self.phone, email: self.email, telegram: self.telegram)}"
  end
...
#puts hash = student1.getInfo.split(", ").map {|i| i.split(": ")}.to_h["fullname"]
